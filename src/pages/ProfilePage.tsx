  ...
  return (
    <div className="fade-in space-y-5 max-w-3xl">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-100 tracking-tight">Profile</h2>
          <p className="text-sm text-slate-400 mt-0.5">Manage your profile and set goals</p>
        </div>
      ...
      <Card className="glow-blue-subtle">
        <CardHeader>
          <CardTitle className="flex items-center gap-2.5">
            ...
            <span className="text-slate-100">Personal Info</span>
          </CardTitle>
          <CardDescription className="text-slate-400">Your basic profile information</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex items-start gap-6">
            <div className="flex flex-col items-center gap-2">
              ...
              <p className="text-[10px] text-slate-500">Avatar</p>
            </div>
            <div className="flex-1 space-y-4">
              <div>
                <Label htmlFor="profile-name" className="text-xs text-slate-300">Display Name</Label>
                <Input id="profile-name" placeholder="Enter your name" value={profile.name} onChange={(e) => update({ name: e.target.value })} className="mt-1.5 bg-slate-800/50 border-slate-700 text-slate-200 placeholder:text-slate-500" />
              </div>
              <div>
                <Label htmlFor="profile-avatar" className="text-xs text-slate-300">Avatar URL</Label>
                <Input id="profile-avatar" placeholder="https://example.com/avatar.jpg" value={profile.avatar} onChange={(e) => update({ avatar: e.target.value })} className="mt-1.5 bg-slate-800/50 border-slate-700 text-slate-200 placeholder:text-slate-500" />
              </div>
            </div>
          </div>
        </CardContent>
        ...
        <CardContent>
          <div className="space-y-4">
            {profile.goals.map((goal, index) => (
              <div key={goal.id}>
                {index > 0 && <Separator className="my-4 bg-slate-700/50" />}
                  ...
                  <div className="space-y-3">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div>
                        ...
                        <Input placeholder="e.g., Read 12 books this year" value={goal.title} onChange={(e) => updateGoal(goal.id, { title: e.target.value })} className="mt-1 bg-slate-800/50 border-slate-700 text-slate-200 placeholder:text-slate-500" />
                      </div>
                      <div>
                        <Label className="text-xs text-slate-300">Target Date</Label>
                        <Input type="date" value={goal.targetDate} onChange={(e) => updateGoal(goal.id, { targetDate: e.target.value })} className="mt-1 bg-slate-800/50 border-slate-700 text-slate-200" />
                      </div>
                    </div>
                    <div>
                      <Label className="text-xs text-slate-300">Description</Label>
                      <Textarea placeholder="Describe your goal..." value={goal.description} onChange={(e) => updateGoal(goal.id, { description: e.target.value })} className="mt-1 min-h-[80px] bg-slate-800/50 border-slate-700 text-slate-200 placeholder:text-slate-500" />
                    </div>
                    <div>
                      <div className="flex items-center justify-between mb-1.5"><Label className="text-xs text-slate-300">Progress</Label><span className="text-xs font-mono text-blue-400">{goal.progress}%</span></div>
      ...
      <Card className="border-slate-700/50">
        <CardHeader><CardTitle className="text-sm text-slate-400">Data Management</CardTitle></CardHeader>
